...
class Synthesizer:
    ...
    def ensure_ingredient_coverage(self, out_lines: List[str], merged_ings: List[Ingredient]) -> List[str]:
        ...
        if "beaten" in all_text:
            # if Eggs were not tokenized earlier, add a canonical token entry
            if not any(name.lower() == "eggs" for name in toks_all_by_name):
                toks_all_by_name["Eggs"] = ["eggs"]
            # If we previously marked Eggs as missing, remove that entry and ensure toks_by_name is consistent
    ...
    def _token_set(self, s: str) -> set:
        """Return normalized token set used for fuzzy matching."""
    ...
    def _normalize_for_dedupe(self, s: str) -> str:
        ...
        # normalize numeric time expressions/ranges (e.g. "4-6 hours", "5 mins") -> placeholder
        s = re.sub(r'\b\d+\s*(?:[-–]\s*\d+)?\s*(?:hours?|hrs?|minutes?|mins?)\b', ' time ', s, flags=re.I)
    ...
    def merge_semantic_steps(self, steps: List[str]) -> List[str]:
        ...
        #
        # --- Ensure heating comes before cooking when cooking references beaten eggs (always run) ---
        try:
            ...
            # If first cook-like step appears before heat, and cook mentions beaten/egg/pour, move heat before it
            if cook_idx is not None and heat_idx is not None and cook_idx < heat_idx:
                ...
                if any(k in cook_text for k in ('beaten', 'beaten eggs', 'beat', 'egg mixture', 'pour')):
                    ...
                    # rebuild lowercase view to keep indices correct for next checks
                    low_merged = [s.lower() for s in merged]
                    print(f"DEBUG: moved heat line from index {heat_idx} to {cook_idx} in merged steps")
        except Exception as _e:
            # non-fatal — don't block merging for unexpected errors
            print("DEBUG: heat/cook reorder check failed:", _e)

        # --- If cook references beaten eggs but there's no explicit 'pour' step, insert a pouring step after heat ---
        try:
            low_merged = [s.lower() for s in merged]
            cook_idx = next((i for i,s in enumerate(low_merged) if re.search(r'\b(cook|fry|bake|simmer|steam)\b', s)), None)
            if cook_idx is not None and 'beaten' in low_merged[cook_idx]:
                ...
                if not has_pour:
                    # place after the first heat/preheat if present, otherwise just before cook_idx
                    heat_idx = next((i for i,s in enumerate(low_merged) if re.search(r'\b(heat|preheat)\b', s)), None)
    ...
    def synthesize(self, top_recipes: List[Recipe], requested_servings: int,
        ...
        if not llm.available():
            ...
            if prep_from_ings:
                ...
                for p in prep_normed[::-1]:
                  # skip inserting if a very similar line already exists (Jaccard overlap >= 0.6)
                    p_key = self._token_set(p)
                    exists_similar = False
                    for s_exist in out_lines:
        ...
        if matches:
            ...
            if min(nums) != 1:
                # model started at something other than 1 — renumber using matches (more robust than splitlines)
                matches_sorted = sorted(matches, key=lambda x: int(x[0]))
                out_lines = []
                for _, text in matches_sorted:
                    text = re.sub(r'\s+', ' ', text).strip()
                    ...
                    if len(text.split()) >= 3:
                        out_lines.append(text)
                print("DEBUG: renumbered out_lines from model-start-at-n:", out_lines)
